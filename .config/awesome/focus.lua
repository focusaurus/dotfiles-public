...
local function run(args)
	awful.spawn.easy_async(args, noop)
end
...
local function by_rules(rules)
	...
	local function tag_and_rules(c)
		...
		for _, tag in pairs(c:tags()) do
			...
			-- log("client", c.name, "has tag", tag.name, "tag_matches", tag_matches, "rule_matches", rule_matches)
...
function module.previous_window()
	awful.client.focus.byidx(-1)
end

function module.next_window()
	awful.client.focus.byidx(1)
end
...
function module.nofi()
	if not by_class("nofi") then
		run(home_bin .. "/nofi")
	end
end
function module.gofi()
...
function notgofi(cli)
	log("focus.previous.notgofi: " .. cli.name)
	return cli.name ~= "tilda"
end

function module.previous()
	log("focus.previous A")
	cli = awful.client.focus.history.get(awful.screen.focused(), 3, notgofi)
	log("focus.previous: " .. cli.name)
	-- if cli.focusable then
	-- log("raising")
	client.focus = cli
	cli:raise()
	-- end
...
function module.browser()
	...
	if firefox then
		...
		if by_rules({ class = "firefox" }) then
			return
		end
		-- if none, launch one
		run("firefox")
		...
		if by_rules({ class = "Google-chrome", name = "main" }) then
			return
		end
		-- fall back to any chrome window
		if by_rules({ class = "Google-chrome" }) then
			return
		end
		-- if none, launch one
...
function module.onepassword()
	if not by_class("1Password") then
		run({ "1password" })
	end
end

function module.obsidian()
	if not by_class("obsidian") then
		run({ "obsidian" })
	end
end
...
function module.calendar()
	if not by_functions(rules_fn({ class = "firefox" }), name_prefix_fn("calendar: ")) then
		run("firefox")
	end
end
...
function module.ghostty()
	if not by_class("ghostty") then
		run({ "ghostty" })
	end
end

function module.alacritty()
	if not by_class("Alacritty") then
		run({ "alacritty" })
	end
end
...
function module.bambustudio()
	if not by_class("BambuStudio") then
		run("/bin/bambu-studio")
	end
end

function module.crealityprint()
	if not by_class("Creality Print") then
		run("/opt/creality-print/CrealityPrint")
	end
end
...
function module.vial()
	if not by_rules({ name = "Vial" }) then
		run("Vial")
	end
end
