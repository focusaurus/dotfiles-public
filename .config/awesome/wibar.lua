...
local mymainmenu = awful.menu({
	items = {
		...
		{ "leader-rofi", focus.rofi },
		{ "leader-nofi", focus.nofi },
...
client.connect_signal("manage", function(c)
	...
	c.urgent = false
	awful.spawn.easy_async({ os.getenv("HOME") .. "/bin/set-icons" }, function() end)
end)

local microphone_script_widget = awful.widget.watch(os.getenv("HOME") .. "/bin/widgets/microphone", 2)
small(microphone_script_widget)
local sound_widget = awful.widget.watch(os.getenv("HOME") .. "/bin/widgets/sound", 2)
small(sound_widget)
local battery_script_widget = awful.widget.watch(os.getenv("HOME") .. "/bin/widgets/battery", 30)
