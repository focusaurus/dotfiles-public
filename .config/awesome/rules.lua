...
awful.rules.add_rule_source("focusaurus", function(c, properties)
	...
	if string.match(c.name, "trello: ") then
		properties.tags = { "2" }
	end
	if string.match(c.name, "FRC: ") then
		properties.tags = { "2" }
	end
	if string.match(c.name, "FRC misc:") then
		properties.tags = { "2" }
	end
end)
