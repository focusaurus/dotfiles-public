...
function module.dev2()
  if prespawned_client ~= nil then
    log('dev2 found prespawn. Showing.')
  ...
  log('dev2: not found. Spawning visible.')
...
function module.unmanage(c)
  log('leader-nofi.unmanage called ')

  if awful.rules.match(c, leader_rules) then
    log('leader-nofi.unmanage: class matches. spawn fresh below')
    awful.spawn.easy_async(leader_path, noop)
  end
end

function module.manage(c)
  log('leader-nofi.manage called ' .. c.name)
  if awful.rules.match(c, leader_rules) then
    log('marking', c.name, 'as not urgent')
    tag4.urgent = false
    c.urgent = false
    -- awful.urgent.delete(c)
    log('done marking urgent')
    -- prespawned_client = c
  end
end
