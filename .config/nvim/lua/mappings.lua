...
vim.keymap.set('n', '<leader>pp', ':w<cr>:silent !pretty-print-files %<cr>:edit!<cr>', { noremap = true })
...
vim.keymap.set('n', '<leader>zp', ':call fzf#vim#complete#path("fd")<cr><cr>', { noremap = true })
-- vim.keymap.set('n', '<leader>zf', ':Files<cr>', { noremap = true })
vim.keymap.set('n', '<leader>zf', ':call fzf#run({"source": "fd --type file", "sink": "tabedit"})<cr><cr>', { noremap = true })
...
vim.keymap.set('i', '<expr> <S-Tab>', [[pumvisible() ? "\<C-p>" : "\<S-Tab>"]], { noremap = true })

-- In insert mode, complete a filesystem path with fuzzy matching (fzf plugin)
vim.keymap.set('i', '<expr> <c-x><c-f>', 'fzf#vim#complete#path("fd")', { noremap = true })
